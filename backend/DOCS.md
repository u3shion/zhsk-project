...
Диспетчер уведомлений. Отправляет сообщения жильцам через выбранный ими канал: **email** (рабочий), **VK** и **SMS** (стабы, готовы к реализации). Хранит полный лог всех отправок. Запускает ежедневный планировщик (APScheduler, 08:00 МСК), который проверяет приближающиеся даты поверки счётчиков и рассылает напоминания.
...
| `VK_ACCESS_TOKEN`    | Токен VK (стаб, не используется до реализации)        |
